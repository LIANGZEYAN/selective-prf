...
Output Passage A or Passage B:"""
        ...
        if self.config.model_type == 't5':
        ...
        elif self.config.model_type in ('llama', 'qwen2', 'qwen3', 'mistral', 'mistral3'):
            ...
            # Mistral3 stores the chat template in the processor, not the tokenizer
            if self.config.model_type == 'mistral3' and not self.tokenizer.chat_template:
            ...
            if device == 'cuda' and self.config.model_type in ('qwen3', 'mistral3'):
            ...
            if self.config.model_type == 'mistral3':
                # Mistral3 is a multimodal model — load and extract text backbone
    ...
    def compare(self, query: str, docs: List):
        ...
        if self.config.model_type == 't5':
            ...
            # Qwen3 supports enable_thinking; disable it for pairwise to get direct answers
            chat_kwargs = dict(tokenize=False, add_generation_prompt=True)
            if self.config.model_type == 'qwen3':
                chat_kwargs['enable_thinking'] = False

            prompt0 = self.tokenizer.apply_chat_template(conversation0, **chat_kwargs)
            prompt0 += " Passage:"
            prompt1 = self.tokenizer.apply_chat_template(conversation1, **chat_kwargs)
            prompt1 += " Passage:"

            tokenized = self.tokenizer([prompt0, prompt1], return_tensors="pt", padding=True)
