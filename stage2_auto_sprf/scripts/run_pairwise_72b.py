#!/usr/bin/env python3
"""
Run Qwen-72B API pairwise reranking: DCG ratio, MRR ratio, Majority@k (top-5).
...
def main() -> None:
    logger = setup_logging("run_pairwise_72b")
    ...
    logger.info("Initialising Qwen-72B API reranker...")
    ...
    for qid in tqdm(query_ids, desc="Qwen-72B Pairwise"):
    ...
    for name, results in [
        ("Qwen-72B-DCG", results_dcg),
        ("Qwen-72B-MRR", results_mrr),
        ("Qwen-72B-Majority@k", results_uw),
    ]:
        ...
        metrics = evaluate_all(labels_df, scores, name, "posthoc", f"Qwen-72B {name.split('-')[-1]}")
        append_to_summary(metrics)
        logger.info("%s — Acc=%.4f  AUC=%.4f", name, metrics["acc"], metrics["auc"])

    logger.info("Qwen-72B pairwise complete.")
